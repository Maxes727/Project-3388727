...
public class AionGameCrypter implements ProtocolCrypter
{
    ...
    byte[] staticKey = "nKO/WctQ0AVLbpzfBkS6NevDYT8ourG5CRlmdjyJ72aswx4EPq1UgZhFMXH?3iI9".getBytes(); // 3.5 Verified
    ...
    public boolean decrypt(byte[] raw, packetDirection dir)
    {
        ...
        else
        {
            if (clientPacketkey == null)
            {
                if(!validatePacket(raw, (byte)0x6D))
            ...
            if(!validatePacket(raw, (byte)0x46))
            	PacketSamurai.getUserInterface().log("Invalid Packet!!!");
            decodeOpcodec(raw);
        }
        return true;
    ...
    private boolean searchKey(byte[] raw, packetDirection dir)
    {
        ...
        if (dir ==  packetDirection.serverPacket && "SM_KEY".equals(packet.getName()))
        {
            ...
            key = (key - 0x3FF2CCC7) ^ 0xCD92E4D7; // 4.0 Verified
            
            clientPacketkey = new byte[8];
            clientPacketkey[0] = (byte) ((key >> 0) & 0xff);
    ...
    {
        return raw[0] == ~raw[3] && raw[1] == ~raw[4] && raw[2] == code;
    }

    private final void decodeOpcodec(byte[] raw)
    {
    	int opcode = (((raw[0] &0xff) << 0) | ((raw[1] &0xff) << 8));
    	opcode = ((opcode ^ 0xD7) - 0xC7); // 4.0 Current
    	
    	raw[0] = (byte)(opcode >> 0 &0xff);
    	raw[1] = (byte)(opcode >> 8 &0xff);
    }
